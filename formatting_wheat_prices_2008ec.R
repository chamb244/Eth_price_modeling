# formatting wheat price data for 2008 EC (2015-2016)

setwd("C:/DATA/Ethiopia/price_modeling/Kevin/EthPriceModelling/output/ETHPriceData/")
list.files()
df <- read.csv("6.Retail price 2008 E.C_wheat.csv")
summary(df) 
names(df) 
head(df)
df <- df[-c(1,3),-c(1)]
...
# colnames(df)[colnames(df) == "UNMILLED.CEREALS.GRAINS_Wheat.White"] <- "pkg.wht.white"
# colnames(df)[colnames(df) == "UNMILLED.CEREALS.GRAINS_Wheat.Mixed"] <- "pkg.wht.mixed"
# colnames(df)[colnames(df) == "UNMILLED.CEREALS.GRAINS_Wheat.Black..Red."] <- "pkg.wht.blred"
...
write.csv(df, file = "reformatted_pkg_wht_2008_ec.csv")
