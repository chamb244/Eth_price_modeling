# formatting wheat price data for 2006 EC (2013-2014)

setwd("C:/DATA/Ethiopia/price_modeling/Kevin/EthPriceModelling/output/ETHPriceData/")
list.files()
df <- read.csv("4.Retail price 2006 E.C_wheat.csv")
summary(df) 
names(df) 
head(df)
df <- df[-c(1,3),-c(1)]
...
# these are all for September, so set year to 2013 for consistency with other month-year combinations here, 
# although note that new year should begin on Sept 
...
write.csv(df, file = "reformatted_pkg_wht_2006_ec.csv")
